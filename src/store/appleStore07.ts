...
export const increment = () => {
  useAppleStore.setState((state) => ({count: state.count + 1}))
}
...
export const getTotal = () => {
  return useAppleStore.getState().count * useAppleStore.getState().price
}

export const doubleCount = async () => {
  const rate = await Promise.resolve(2); // 模拟异步操作

  useAppleStore.setState((state) => ({count: state.count * rate}))
}
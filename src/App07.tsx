...
import useAppleStore, {increment, decrement, getTotal, doubleCount} from "./store/appleStore07";
...
const Child1 = () => {
  ...
  return (
    <div>
      <h1>zustand：setState、getState</h1>
      <hr />
      <div>单价：{price}</div>
      <div>数量：{count}</div>
      <div>总价：{getTotal()}</div>
      <div>getState获取:{ useAppleStore.getState()?.count}</div>
      <div>
        <button onClick={increment}>数量+1</button>{" "}
        <button onClick={decrement}>数量-1</button>
      </div>
      <div>
        <button onClick={doubleCount}>双倍价格</button>
        <button onClick={myLocalAction}>组件独有的+3操作</button>
      </div>
    </div>
  );
};
